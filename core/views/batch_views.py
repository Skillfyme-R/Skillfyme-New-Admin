...
CHANGE vs original:
  • create_batch: optionally accepts instructor_id in the POST payload and links
    the Instructor FK on the new Batch. All other logic is unchanged.
  • _batch_out_data: unchanged (instructor_name/instructor_email still returned).
...
# CHANGE: now optionally sets batch.instructor FK from instructor_id field
# ---------------------------------------------------------------------------
...
def create_batch(request):
    ...
    # ── NEW: resolve optional instructor FK ──────────────────────────────────
    instructor_instance = None
    instructor_id = request.data.get('instructor_id')
    if instructor_id:
        try:
            from instructors.models import Instructor
            instructor_instance = Instructor.objects.get(pk=instructor_id, status='active')
            # Auto-fill legacy text fields from the Instructor record
            if not data.get('instructor_name'):
                data['instructor_name'] = instructor_instance.name
            if not data.get('instructor_email'):
                data['instructor_email'] = instructor_instance.email
        except Exception:
            logger.warning('instructor_id %s not found or inactive — skipping FK', instructor_id)
    # ── END NEW ──────────────────────────────────────────────────────────────
    ...
    batch = Batch.objects.create(
        ...
        instructor=instructor_instance,          # NEW — FK (None if not provided)
    )
...
# ---------------------------------------------------------------------------
# Batch detail / update / delete — /api/batch/<batch_code>/  (unchanged)
# ---------------------------------------------------------------------------
...
def _delete_batch(request, batch_code):
    ...
    scheduler_service.remove_batch_jobs(batch_code)
    batch.delete()
    return Response({'detail': f"Batch '{batch_code}' deleted successfully."})


# ---------------------------------------------------------------------------
# List batches — GET /api/batches/  (unchanged)
# ---------------------------------------------------------------------------
...
def list_batches(request):
    ...
    result = []
    for b in batches:
        learner_count = Learner.objects.filter(batch_id=b.batch_code).count()
        out = BatchOutSerializer(_batch_out_data(b, learner_count))
        result.append(out.data)
    return Response(result)


# ---------------------------------------------------------------------------
# Upload learners CSV — POST /api/batch/upload-learners/  (unchanged)
# ---------------------------------------------------------------------------
